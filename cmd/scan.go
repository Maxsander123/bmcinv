...
scanTimeout   int
scanExport    bool
scanExportDir string
scanExportFmt string
)
...
Examples:
  ...
  bmcinv scan 10.0.0.0/24 --export              # Scan and export to CSV
  bmcinv scan 10.0.0.0/24 -e -o ~/backup        # Scan and export to directory
  bmcinv scan 10.0.0.0/24 -e -F json            # Scan and export as JSON`,
...
scanCmd.Flags().BoolVarP(&scanExport, "export", "e", false, "export to CSV/JSON after scan")
scanCmd.Flags().StringVarP(&scanExportDir, "export-dir", "o", ".", "export output directory")
scanCmd.Flags().StringVarP(&scanExportFmt, "export-format", "F", "csv", "export format: csv or json")
}
...
// Chain export if requested
if scanExport {
fmt.Println("\n--- Exporting data ---")
exportFormat = scanExportFmt
exportOutput = scanExportDir
